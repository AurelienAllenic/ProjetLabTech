...
function FooterLink({ href, label, active }) {
  return (
    <li>
      <Link
        to={href}
      ...
      >
        {active && <span className="inline-block w-1.5 h-1.5 rounded-full bg-raspberry-400 mr-2 mb-0.5 align-middle" aria-hidden="true" />}
        {label}
      </Link>
    </li>
  );
...
export default function Footer() {
  ...
  return (
    <footer role="contentinfo" className="bg-gray-900 text-gray-400 text-sm pt-12 pb-6 mt-auto">
      <div className="max-w-6xl mx-auto px-6 grid sm:grid-cols-2 md:grid-cols-4 gap-8 mb-10">
        ...
        <nav aria-label="Navigation principale du pied de page">
          <h4 className="font-semibold text-white mb-3 text-xs uppercase tracking-widest">Navigation</h4>
          <ul className="space-y-2">
            {NAV_LINKS.map((l) => (
              <FooterLink key={l.href} href={l.href} label={l.label} active={pathname === l.href} />
            ))}
          </ul>
        </nav>
        ...
        <nav aria-label="Liens légaux du pied de page">
          <h4 className="font-semibold text-white mb-3 text-xs uppercase tracking-widest">Légal</h4>
          <ul className="space-y-2">
            {LEGAL_LINKS.map((l) => (
              <FooterLink key={l.href} href={l.href} label={l.label} active={pathname === l.href} />
            ))}
        ...
        {/* Contact */}
        <div>
          <h4 className="font-semibold text-white mb-3 text-xs uppercase tracking-widest">Contact</h4>
      ...
      <div className="border-t border-gray-800 pt-6 text-center">
        <p className="text-xs text-gray-600">© 2025 Lab'IA. Tous droits réservés.</p>
      </div>
    </footer>
  );
}
