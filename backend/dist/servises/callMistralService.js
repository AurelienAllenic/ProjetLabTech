...
function getMistralApiKey() {
    ...
    if (!key?.trim()) {
        throw new Error("Clé API Mistral absente : définissez API_KEY ou MISTRAL_API_KEY dans backend/.env");
    }
    return key.trim();
}
/** Corps d'erreur Mistral : souvent `{ message, type, code }` à la racine, pas `{ error: { message } }`. */
function mistralErrorMessage(status, body, rawText) {
...
export async function generateTextFromPdf(pdfText) {
    ...
    try {
        ...
        console.error("[Mistral] Réponse non-JSON (HTTP %s): %s", response.status, rawText.slice(0, 500));
