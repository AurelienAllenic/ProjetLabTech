...
type ParsedJson =
  | { elements?: ApiElement[]; conclusion?: unknown }
  | ApiElement[]
  | ApiElement;
...
function extractConclusion(parsed: ParsedJson): string | undefined {
  if (!parsed || Array.isArray(parsed)) return undefined;
  if (typeof parsed !== "object") return undefined;
  const raw = (parsed as { conclusion?: unknown }).conclusion;
...
export async function analysePdf(req: Request, res: Response): Promise<void> {
  try {
    ...
    const result: AnalysisResult["result"] = { elements };
    if (conclusion) {
      result.conclusion = conclusion;
    }
    res.json({ success: true, result } satisfies AnalysisResult);
  } catch (err) {
